...
Swagger: [http://localhost:4001/docs/](http://localhost:4001/docs/). Канвас: [http://127.0.0.1:5174/](http://127.0.0.1:5174/). Спецификация: [http://localhost:4001/openapi.json](http://localhost:4001/openapi.json) и [файл в репозитории](docs/openapi.json).
...
```text
apps/api/             бэкенд
apps/web/             фронтенд (React + Vite + React Flow)
packages/contracts/   схемы API и типы TypeScript
docs/                 задание и документация
scripts/              проверки
```

Проект использует npm workspaces. Фронтенд — `@canvas/web`. `npm run dev` поднимает API и канвас вместе.
